...
1. Edit the `conf/env.cfg` file and set required variables.
   ...
   *   Amazon FSx for NetApp ONTAP handling
       ...
       >
       > The purpose of these steps is to make sure that ONTAP stroage is mounted correctly on the X-Spot worker node. In the [step of X-Spot Validation](x-spot-validation.md), you can login to the worker node with ssh or Session Manager, and check if all ONTAP storage folders are mounted correctly.
       >
